...
from pydanticai_api_template.agents.rag.orchestrator import run_agentic_rag
...
async def verify_api_key(api_key: str = Depends(api_key_header)) -> bool:
    ...
    if not PYDANTICAI_API_KEY:
        ...
        logger.warning("API key validation skipped - no PYDANTICAI_API_KEY set in environment")
        return True
...
async def chat_with_agent(chat_message: ChatMessage, authorized: bool = Depends(verify_api_key)) -> ChatResponse:
    ...
    with logfire.span("chat_with_agent", operation_type="api_chat", model="gpt-4o") as span:
        span.set_attributes(
            {
                "message_length": len(chat_message.message),
                "user_message": chat_message.message[:100] + ("..." if len(chat_message.message) > 100 else ""),
...
async def generate_story_idea(chat_message: ChatMessage, authorized: bool = Depends(verify_api_key)) -> StoryIdea:
    ...
    with logfire.span("generate_story_idea", operation_type="creative_generation", model="gpt-4o") as span:
        ...
        try:
            logger.debug(f"Running PydanticAI story agent for message: " f"{chat_message.message[:50]}...")
            ...
            enhanced_prompt = (
                f"Generate a creative and original story idea based on this input: " f"{chat_message.message}"
            )
...
@app.post("/agentic-rag", response_model=RagResponse)
async def agentic_rag(
    request: RagRequest,
    authorized: bool = Depends(verify_api_key),
) -> RagResponse:
    """Endpoint for agentic RAG: retrieve docs and answer user question."""
    with logfire.span(
        "agentic_rag",
        operation_type="agentic_rag",
        model=os.getenv("PRIMARY_MODEL", "openai:gpt-4o-mini"),
    ) as span:
        span.set_attributes({"question_length": len(request.question)})
        try:
            answer = await run_agentic_rag(request.question)
            span.set_attributes({"success": True})
            return RagResponse(answer=answer)
        except Exception as e:
            logger.exception(f"Error in agentic_rag endpoint: {e}")
            span.set_attributes({"error": True, "error.message": str(e)})
            raise HTTPException(
                status_code=500,
                detail="An internal error occurred while processing your RAG request.",
            )
